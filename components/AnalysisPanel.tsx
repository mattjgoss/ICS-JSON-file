...
import { AlertTriangle, CheckCircle, Info, DollarSign, Calendar, FileText } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-white border-l border-slate-200 overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        <h2 className="font-semibold text-slate-800 flex items-center gap-2">
          <Info className="w-5 h-5 text-sap-blue" />
          ICS Explainer
        </h2>
        <button
          onClick={onAnalyze}
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors shadow-sm
            ${isLoading 
              ...
              : 'bg-sap-blue text-white hover:bg-sap-dark'
            }`}
        >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
        ...
        {!analysis && !isLoading && !error && (
          <div className="text-center py-12">
            <p className="text-slate-500">Click "Analyze JSON" to get AI-powered insights about this SAP Concur payload.</p>
          </div>
        )}
                ...
                {analysis.fieldExplanations.map((item, idx) => (
                  <div key={idx} className="border border-slate-200 rounded-lg p-3 hover:shadow-sm transition-shadow">
                    <div className="flex justify-between items-start mb-1">
                      <div className="font-mono text-sm text-sap-blue font-medium">{item.field}</div>
